...
if (isset($_GET['financeType']) && isset($_GET['vehicleType']) && isset($_GET['modelSelection']) && isset($_GET['brandSelection'])) {
  ...
  $finance = ($_GET['financeType'] == 'personal')? 'personal' : 'business'; // a form of query sanitisation
	...
	$manufacturer = htmlspecialchars(dsg_decode($_GET['brandSelection']));
	$model = htmlspecialchars(dsg_decode($_GET['modelSelection']));
  ...
  // require_once('includes/SearchBarCap.php');
    ...
    if (mysql_num_rows($brand_notes_qry)) {	
	    $brandInfo = mysql_fetch_assoc($brand_notes_qry);
			// $img = $_SERVER['DOCUMENT_ROOT'].'/images/cap-vehicles/'.$brandInfo['imageid'].'.jpg';
		  // if (file_exists($img)) {
			//  $with_img = TRUE;
			//  print '<div id="vehicle-image"><img alt="'.$manufacturer.'" src="/images/cap-vehicles/'.$brandInfo['imageid'].'.jpg" /></div>';
		  // }
